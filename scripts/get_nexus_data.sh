...
for dir in "${directories[@]}"; do
    ...
    if [[ $fextension =~ nex ]]; then
        ...
        if [[ $fextension =~ fasta ]]; then
            for files in $basedir/$dir/*.fasta; do
                for file in $files; do
                    if [[ $file != $basedir/$dir/full_alignment.fasta ]]; then
                        echo "Skipping $file"
                        continue
                    fi
                    if [[ -f $file ]]; then
                        taxa=$( grep -r ">" $file | wc -l )
                        secondtaxon=$(( $( grep -r ">" $file -n | awk 'NR == 2' | cut -d ":" -f 1 ) - 1 ))
                        sequences=$(sed -n "2,$(($secondtaxon))p" $file | tr -d '[:space:]' | wc -m)
                        if [[ -n $taxa && -n $sequences ]]; then
                            relative_filepath=${file#$basedir/}
                            echo "writing as $relative_filepath"
                            # Append data to output file
                            echo "$relative_filepath,$taxa,$sequences" >> "$output_file"
                        fi
                    fi
            ...
            echo "Error in config file: unrecognized file extension"
            exit 0
        fi
    fi
done
