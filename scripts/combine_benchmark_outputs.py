...
files_to_join = [x for x in os.listdir(benchmarking_folder)]
seq_ids = [
    record.id for record in SeqIO.parse(subdir + "/full_alignment.fasta", "fasta")
]
...
for filename in files_to_join:
    ...
    if rulename not in files_met:
        if rule_for_sequence:
            df = pd.concat(
                (
                    pd.read_csv(benchmarking_folder + ff, sep="\t")
                    for ff in files_to_join
                    if rulename in ff
                ),
                ignore_index=True,
            )
...
def bar_plot_breakdown(dataframe, colname, filename):
    ...
    ax.bar_label(
        ...
        labels=[
            "{:d}calls\n{:.3f} per run".format(
                x, float(list(thecol)[i]) / float(x if x != 0 else 1)
            )
