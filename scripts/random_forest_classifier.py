    ...
    def train_random_forest_classifier(df, column_name):
        ...
        def objective(trial):
            ...
            min_samples_split = trial.suggest_float(
                "min_samples_split", 0.00001, 1, log=True
            )
            min_samples_leaf = trial.suggest_float(
                "min_samples_leaf", 0.00001, 1, log=True
            )
        ...
        for i in range(num_classifiers):
            ...
            X_large_subset = X_large.sample(
                n=subset_size, replace=False, random_state=42
            )
        ...
        X_test_df = pd.concat(
            [X_test_imputed_df.reset_index(drop=True), y_test_df], axis=1
        )
        ...
        balanced_test_df = (
            pd.concat([subsampled_group_0, subsampled_group_not_0])
        ...
        model_results = pd.DataFrame({"actual": y_test_balanced})
        ...
        probabilities = np.array(
            [clf.predict_proba(X_test_balanced)[:, 1] for clf in classifiers]
        )
        ...
        feature_importances = np.array(
            [clf.feature_importances_ for clf in classifiers]
        )
        avg_importances = np.mean(feature_importances, axis=0)
        feature_names = X_train_imputed_df.columns
        pd.DataFrame({"model_importance": avg_importances}, index=feature_names).to_csv(
            model_features_csv
        )
        ...
        for i in range(len(model_importances)):
            pd.DataFrame(
                {"model_importance": model_importances[i]}, index=feature_names
            ).to_csv(
                data_folder
                + "model_features_"
                + column_name
