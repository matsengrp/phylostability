...
for sd in $maindir/*/; do
  if [ -f $sd/$larchinputname ]; then # check if there's a larch input file
    if ! [ -f $sd/$larchoutputname ]; then # check if there's a larch output file already
      # now run larch on the specified input file
      logfilepath=$sd/larch-usher-log
      larch_usher_options=" -i $sd/$larchinputname -c $iterations -o $sd/$larchoutputname --sample-method rf-maxsum"
      if [ -f $sd/$vcfname ]; then
        larch_usher_options="-i $sd/$larchinputname -v $sd/$vcfname -c $iterations -o $sd/$larchoutputname --sample-method rf-maxsum"
      fi
